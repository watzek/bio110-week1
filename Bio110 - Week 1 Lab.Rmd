...
Note in the above examples that quotation marks are used to define a, but not myFavoriteNumber. This is beacuse variables can have different types, and quotes (or lack of quotes) can help indicate what kind of type your variable is. Generally quotes are required for string (e.g. text) varaibles, while quotes are not required for integers, functions, or setting one variable equal to another.
...
#and one for the y-values, heartbeats (in order)
y1=c()
...
We can add additional information to the plot, including specific x and y axis limits, axes labels, and a title. Make sure you update the "main" text value so it contains your name.
...
Below is a series of commands that adds a best fit line. It may seem a little strange that the "abline" command isn't connected to the plot command, but still affects the plot output. In R, once the "plot" command is run, it assumes that subsequent plot-related commands (like "abline") should be applied to that plot. 
...
You can plot a second set of values by first creating another vector. Below, create a vector (y2) that has your heartbeat count values while exercising. They should be at the same time intervals as x.
...
Now we can use the "matplot" function, which lets you plot multiple columns or sets of data. Make sure you update the "main" section to include your name.

```{r}

matplot(x=x, y=cbind(y1,y2), type="p", col=c("red", "green"), pch=c(16,16), main="Student's heart rate increases with exercise", xlab="time in seconds", ylab="resting beats-red and exercising beats-green")

```


Now we want to add best-fit lines to both data sets y1 and y2. The procedure for this is nearly identical to what we did before. Instead of using "plot", we'll use "matplot" since we're comparing two data sets. Also, we'll create a model and abline for each vector:
